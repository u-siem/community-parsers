...
static PALOALTO_SCHEMA : OnceLock<FieldSchema> = OnceLock::new();

pub fn paloalto_field_schema() -> FieldSchema {
    FieldSchema {
        fields: {
...
impl LogParser for PaloAltoFWParser {
    ...
    fn schema(&self) -> &FieldSchema {
        PALOALTO_SCHEMA.get_or_init(|| paloalto_field_schema())
    }
...
impl LogGenerator for PaloAltoLogGenerator {
    ...
    fn configure(&mut self, _config: usiem::prelude::GeneratorConfig) {
        todo!()
    }
}
