...
pub fn parse_log(log: SiemLog) -> Result<SiemLog, SiemLog> {
    ...
    match observer_ip {
        Some(val) => {
            log.add_field("observer.ip", SiemField::IP(val));
        }
        None => {}
    };
...
mod filterlog_tests {
    ...
    fn test_parser() {
        ...
        assert_eq!(
            &SiemField::from_str("SERIALNUMBER111"),
            log.field("observer.id").unwrap(),
        );
        assert_eq!(None, log.field("observer.name"));
        assert_eq!("127.0.0.1", log.origin(),);
        assert_eq!(
            &SiemField::from_str("98:90:96:de:f1:78"),
            log.field("source.mac").unwrap(),
        );
        assert_eq!(
            &SiemField::from_str("ec:f4:bb:fb:f7:f6"),
            log.field("destination.mac").unwrap(),
        );
        assert_eq!(
            &SiemField::IP(SiemIp::V4(ipv4_from_str("10.1.2.3").unwrap())),
            log.field("source.ip").unwrap(),
        );
        assert_eq!(
            &SiemField::IP(SiemIp::V4(ipv4_from_str("10.2.3.4").unwrap())),
            log.field("destination.ip").unwrap(),
        );
    }
    #[test]
    fn test_parser_2() {
        ...
        let log = SiemLog::new(log.to_string(), 0, "127.0.0.1");
        let log = parse_log(log).unwrap();
        assert_eq!("firewall", log.service());
    ...
    fn test_parser_web_connection() {
        ...
        assert_eq!(
            &SiemField::from_str("18B1690729A8"),
            log.field("observer.id").unwrap()
        );
        assert_eq!(
            &SiemField::from_str("FWSonicWall"),
            log.field("observer.name").unwrap(),
        );
        assert_eq!("127.0.0.1", log.origin());
        assert_eq!(
            &SiemField::from_str("98:90:96:de:f1:78"),
        ...
        assert_eq!(
            &SiemField::IP(SiemIp::from_ip_str("192.168.168.10").unwrap()),
            log.field("source.ip").unwrap()
        );
        assert_eq!(
            &SiemField::IP(SiemIp::V4(ipv4_from_str("69.192.240.232").unwrap())),
            log.field("destination.ip").unwrap()
        );
        assert_eq!(
            &SiemField::from_str("www.suntrust.com"),
            log.field("url.domain").unwrap()
        );
        assert_eq!(
            &SiemField::from_str("/favicon.ico"),
            log.field("url.path").unwrap(),
        );
        match log.event() {
