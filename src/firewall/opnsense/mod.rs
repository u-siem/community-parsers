use usiem::chrono::prelude::{DateTime, Datelike, NaiveDateTime, TimeZone, Utc};
use std::borrow::Cow;
...
pub fn parse_log(log: SiemLog) -> Result<SiemLog, SiemLog> {
    ...
    let naive = NaiveDateTime::from_timestamp_millis(log.event_received()).unwrap_or_default();
...
pub fn timestamp_to_i64(timestamp: &str) -> i64 {
    ...
    let dt = timestamp.parse::<DateTime<Utc>>().unwrap_or_default();
...
mod filterlog_tests {
    ...
    fn parse_basic_logs() {
        ...
        let log_event = SiemLog::new(log.to_owned(), 1000000 as i64, "127.0.00.1");
