...
use usiem::prelude::LogParsingError;

pub fn parse_log_combinedio(mut log: SiemLog) -> Result<SiemLog, LogParsingError> {
    ...
    let http_method = parse_http_method(http_method);

    let (http_protocol, http_version) = match version.find('/') {
        Some(p) => (parse_http_protocol(&version[..p]), &version[(p + 1)..]),
    ...
    let user_agent = Cow::Owned(user_agent.to_string());
    let url_path = Cow::Owned(url_path.to_string());
    let url_query = Cow::Owned(url_query.to_string());
    let url_extension = Cow::Owned(url_extension.to_string());
    let url_full = Cow::Owned(url.to_string());
    let referer = match referer {
        "" => None,
        _ => Some(SiemField::Text(Cow::Owned(referer.to_string()))),
    };
    ...
    if let Some(referer) = referer {
        log.add_field("http.request.referrer", referer);
    }
    Ok(log)
}
...
mod filterlog_tests {
    ...
    fn test_parse_combinedio() {
        ...
        let log = SiemLog::new(log.to_string(), 0, "127.0.0.1");
        ...
        match siem_log {
            Ok(log) => {
                ...
                assert_eq!(
                    ...
                    Some(&SiemField::U32(304))
                );
                assert_eq!(
                    log.field(field_dictionary::SOURCE_BYTES),
                    Some(&SiemField::U32(164))
                );
                assert_eq!(
                    log.field(field_dictionary::DESTINATION_BYTES),
                    Some(&SiemField::U32(465))
                );
                assert_eq!(
