...
use usiem::prelude::{LogParser, LogParsingError};

static APACHE2_SCHEMA : OnceLock<FieldSchema> = OnceLock::new();
static MODSECURITY_SCHEMA : OnceLock<FieldSchema> = OnceLock::new();

pub fn apache2_field_schema() -> FieldSchema {
    FieldSchema {
        fields: {
...
pub fn modsecurity_field_schema() -> FieldSchema {
    FieldSchema {
        fields: {
            let mut fields = BTreeMap::new();
            fields.insert(
...
impl LogParser for Apache2Parser {
    ...
    fn parse_log(&self, log: SiemLog, _datasets : &DatasetHolder) -> Result<SiemLog, LogParsingError> {
        httpd::parse_log_combinedio(log)
    }
    /// Name of the parser
    ...
    fn schema(&self) -> &FieldSchema {
        APACHE2_SCHEMA.get_or_init(|| apache2_field_schema())
    }

    fn generator(&self) -> Box<dyn usiem::prelude::LogGenerator> {
        todo!()
    }
}
...
impl LogParser for Apache2ModSecurityParser {
    ...
    /// Get parser schema
    fn schema(&self) -> &FieldSchema {
        MODSECURITY_SCHEMA.get_or_init(|| modsecurity_field_schema())
    }
